...
import function.VandermondeInterpolationFunction;
...
public class AddVandermondeInterpolationFunctionInterface implements ActionListener
{
// --------------------------------
// Attributs:
//--------------------------------
	// Parametres
	private final String			title		= "Vandermonde Interpolation Function Creation";
...
//Constructeur:
//--------------------------------
		public AddVandermondeInterpolationFunctionInterface( Application app)
		{
			this.app = app;
		}
...
//--------------------------------
		...
		{
			// Init frame
			GridLayout disposition	= new GridLayout(17, 1);
			...
			this.degresArea			= new JTextField();
			this.degresArea			.setBorder(BorderFactory.createLineBorder(Color.black, 1));
			pan						.add(new JLabel());
			...
			pan						.add(new JLabel());
			pan						.add(new JLabel("Degres Of Interpolation"));
			pan						.add(degresArea);
			pan						.add(new JLabel());
...
// --------------------------------
		private boolean loadInterpolationFunction(Scanner sc)
		{
			...
			try
			{
				...
				new VandermondeInterpolationFunction(X, Y, "", deg);
			}
		...
		private class OkBL implements ActionListener
		{
			public void actionPerformed(ActionEvent arg0)
			{
				...
				try
				{
					...
					int deg			= Integer.parseInt(degresArea.getText());
					for (int i = 0; i<nbrPoint; i++)
					{
						X[i] = Double.parseDouble(tabl.getElement(0, i));
						Y[i] = Double.parseDouble(tabl.getElement(1, i));
					}
					VandermondeInterpolationFunction f = new VandermondeInterpolationFunction(X, Y, func, deg);
